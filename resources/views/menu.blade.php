    ...
    <div id="menu-grid" class="menu-container">
        @forelse($menuItems as $item)
            <div class="card {{ $item->category === 'food' ? '' : 'hidden' }} {{ $canOrder ? 'interactive' : 'readonly' }}" 
                 ...
                 @if($canOrder) onclick="placeOrder(this)" @endif>
                 
                @if($item->image) <img src="{{ $item->image }}" alt="{{ $item->name }}" class="card-img">
                @else <div class="card-img" style="display: flex; align-items: center; justify-content: center; color: #999;">No Image</div> @endif
                ...
                <div class="card-body">
                    ...
                    @if($canOrder) <small style="color:green; display:none; margin-top:5px; font-weight:bold;" class="added-msg">Added to Bill!</small> @endif
                </div>
            </div>
        @empty
    ...
    <script>
        ...
        async function placeOrder(element) {
            ...
            try {
                ...
                // 2. CHECK IF SERVER SAID YES OR NO
                if (!response.ok) {
                    const errorText = await response.text();
                    alert("FAILED! Server said: " + response.status + "\n" + errorText);
                    return; // Stop here
                }

                // 3. Only show "Added" if server actually saved it
